...
static void dpdk_print(pktio_entry_t *pktio_entry)
{
	...
	len += snprintf(&str[len], n - len, "DPDK pktio config:\n");
	len += snprintf(&str[len], n - len, "  multicast             %d\n", opt->multicast_enable);
	len += snprintf(&str[len], n - len, "  num_rx_desc           %d\n",
			opt->num_rx_desc_default);
	len += snprintf(&str[len], n - len, "  num_tx_desc           %d\n",
			opt->num_tx_desc_default);
	len += snprintf(&str[len], n - len, "  rx_drop_en            %d\n", opt->rx_drop_en);
	len += snprintf(&str[len], n - len, "  min_rx_burst          %d\n", opt->min_rx_burst);
	len += snprintf(&str[len], n - len, "  tx_offload_multi_segs %d\n",
			opt->tx_offload_multi_segs);
